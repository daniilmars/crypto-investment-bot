...
def load_config():
    ...
    except (FileNotFoundError, yaml.YAMLError): return None
...
def save_whale_transactions(transactions: list):
    ...
    conn.commit()
    conn.close()
    print(f"Processed {len(transactions)} transactions for the database.")
...
def get_whale_transactions(min_value_usd: int = 1000000):
    ...
    if not api_key or api_key == "YOUR_WHALE_ALERT_API_KEY":
        print("Error: Whale Alert API key is not configured.")
        return None
    ...
    try:
        ...
        if data.get('result') == 'success':
            ...
            print(f"Successfully fetched {len(transactions)} whale transactions.")
            save_whale_transactions(transactions) # Save the data
            return transactions
        else:
            print(f"Whale Alert API error: {data.get('message')}")
            return None
    except requests.exceptions.RequestException as e:
        print(f"Error fetching from Whale Alert API: {e}")
        return None

if __name__ == '__main__':
    print("--- Testing Whale Alert Data Collector (with DB saving) ---")
