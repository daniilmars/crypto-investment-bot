...
def _score_single_batch(client, articles: list) -> list | None:
    ...
    prompt = (
        ...
        "INSIDER & INSTITUTIONAL FILINGS (special handling — DO weight these):\n"
        "- Articles from source 'OpenInsider' or '13F:*' are SEC filings, not news. "
        "They show what corporate insiders and major hedge funds actually DID with "
        "their own capital. Score these on conviction, not surprise.\n"
        "- Form 4 insider BUYS >$500K by C-suite (CEO, CFO) → +0.5 to +0.7. "
        "Cluster buys (multiple insiders within days) → +0.7 to +0.9.\n"
        "- Form 4 insider SELLS → score -0.2 to -0.4 unless it's a planned 10b5-1 "
        "trade or routine vesting (then 0.0).\n"
        "- 13F NEW positions or major increases (>50%) by smart-money funds "
        "(Berkshire, Bridgewater, Tiger Global, etc.) → +0.4 to +0.6.\n"
        "- 13F position TRIMS (>30% reduction) → -0.3 to -0.5.\n"
        "- Filings are public knowledge but slow-moving signal: trust them when "
        "they corroborate other catalysts.\n\n"
        f"Articles:\n{articles_text}\n\n"
